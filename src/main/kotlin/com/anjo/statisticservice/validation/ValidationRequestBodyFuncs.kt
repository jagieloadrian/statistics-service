...
fun isEpidemicValid(epidemicDto: EpidemicDto): Pair<Boolean, List<String>> {
    ...
    validField(
        isGreaterThanZero(runId), "RunId $MUST_BE_GREATER_THAN_ZERO",
        reasons
    ) { res -> result = res }
    validField(
        isGreaterThanZero(generation), "Generation $MUST_BE_GREATER_THAN_ZERO",
        reasons
    ) { res -> result = res }
    validField(
        isGreaterThanZero(population), "PopulationSize $MUST_BE_GREATER_THAN_ZERO",
        reasons
    ) { res -> result = res }
    validField(
        isGreaterThanZero(mobilityMul), "Mobility multiplier $MUST_BE_GREATER_THAN_ZERO",
        reasons
    ) { res -> result = res }
    validField(
    ...
    validField(
        isGreaterThanZero(susceptible), "Susceptible $MUST_BE_GREATER_THAN_ZERO",
        reasons
    ) { res -> result = res }
